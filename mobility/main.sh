...
start_services() {
    ...
    gnome-terminal -- bash -c "flink/bin/flink run -pyclientexec /usr/bin/python3 -pyexec /usr/bin/python3 -py mobility/FlinkConsumer.py --jarfile connector.jar; exec bash"

    # Running Cassandra.py in a new terminal
    echo "Running Cassandra.py..."
    source ./venv/bin/activate
    gnome-terminal -- bash -c "python $PROJECT_DIR/mobility/Cassandra.py; exec bash"

    # Running KafkaProducer.py in a new terminal
    echo "Running KafkaProducer.py..."
    gnome-terminal -- bash -c "python $PROJECT_DIR/mobility/KafkaProducer.py; exec bash"

    # Activate python environment and run RealTimeAnalysis.py in a new terminal
    echo "Running RealTimeAnalysis.py..."
    cd $PROJECT_DIR/mobility
    gnome-terminal -- bash -c "cd mobility/; python $PROJECT_DIR/mobility/Dashboard.py; exec bash"
}
